...
<script type = "text/javascript">
	...
	$(document).ready(function(){
		...
		$(document).on("click", ".jView", function(){
			...
			location.href=rootPath+"/superOrder/orderView?number="+number+"&rurl="+encryptHexStr(document.URL);
		});
		...
		$(document).on("change", ".jStatus", function(){
			...
			if(status == "PYMT0070") {
				$.ajax({
					url : "/NaeKkot/web/payletter/cancelPayment" ,
					type : "POST",
					dataType : "json" ,
					data : {
						orderNumber : no
		...
		$(".jExcel").click2(function(){
			...
			location.href = rootPath + "/superOrder/downExcel?pageType="+pageType;
		});
	...
	function tictoc(){
		...
		if(sec < 0)	{
			sec = 60;
			$("#jData").ajaxSubmit({
				url : rootPath+"/superOrder/refreshOrderList",
				type : "POST",
				forceSync : true,
				dataType : "HTML",
				success : function(data){
		...
		<table class = "datacList" style="margin-top: 15px;">
			<thead>
				<tr>
					...
					<th>적용금액</th>
					<th>배송사진</th>
					<c:if test="${param.pageType ne 3 }">
					<th>주문상태</th>
					</c:if>
					<c:if test="${param.pageType eq 3 }">
					<th>업무처리(복구)</th>
					</c:if>
				</tr>
			</thead>
			<tbody style = "text-align:center;">
				...
				<c:forEach var="row" items="${retData.list }" varStatus="status">
					<tr>
						...
						<td>${row.FsidoName}&nbsp;${row.FgugunName}<br/>${row.userName }</td>
						<td>${row.sidoName}&nbsp;${row.gugunName}<br/>${row.franchiseName }</td>
						...
						<td>
							<select class="jStatus" name="status" _no="${row.orderNumber }" >
