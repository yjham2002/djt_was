...
<script type = "text/javascript">
	...
	$(document).ready(function(){
		...
		$(".jView").click2(function(){
			...
			location.href=rootPath+"/photoView?productNumber="+no+"&rurl="+encryptHexStr(document.URL);;
...
<form id="jData" method="post" enctype="multipart/form-data">
	<div id="Contents"  class="notice" style="margin: 100px 0px 0px 100px;">
	    <h1>사진방</h1>
		<h2 style="width:98%;">
		</h2>
					...
					<c:forEach var="row" items="${retData.list }" varStatus="status" >
						<li style="float: left; width: 30%; margin: 5px; padding: 5px; border: 1px solid #c0c7cc;">
							...
							<div class="pRight" align="left">
								<p>${row.productCode }</p>
								<p>${row.productName }</p>
								<p class="b"><fmt:formatNumber value="${row.retailPrice}" pattern="#,###" />원</p>
								<p class="b">${row.franchiseName }</p>
								<p class="b">${row.sidoName } ${row.gugunName }</p>
								<p>${row.franchiseDesc }</p>
							</div>
							<input type="button" class="button medium white jView" value="상세보기" _no="${row.productNumber }" style="width: 90%">
						</li>
					</c:forEach>
				</ul>
			</div>
