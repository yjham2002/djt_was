...
//파라미터가 비어있는지 확인
function checkEmpty(_JObj,message){
	var _JObj = arguments[0];
	var message = arguments[1];
	var focusOpt = arguments[2];
	focusOpt = typeof(focusOpt) == null ? false : focusOpt;
	
	if(_JObj.val().length == 0
			|| _JObj.data("hint") == _JObj.val())
	{
		alert(message + " 필드를 입력해주세요!");
		if(focusOpt)
			_JObj.focus();
		return false;
	}
...
function checkRegExp(regExp, _JObj, message){
	var regExp = arguments[0];
	var _JObj = arguments[1];
	var message = arguments[2];
	var focusOpt = arguments[3];
	focusOpt = typeof(focusOpt) == null ? false : focusOpt;
	
	if (!regExp.test(_JObj.val())) { 
		alert(message);
		if(focusOpt)
			_JObj.focus();
		return false;
...
function InitImageUpload(index, controllerName) {
    ...
    var url = '/action_front.php?cmd=BaseAdmin.imagePreUpload';
    ...
    new AjaxUpload('#btnImageUpload' + index, {
        ...
        onComplete: function (file, responseText) {
            ...
            previewImageBind(index, responseText);
        }
    });
}
...
function previewImageBind(index, imgURL) {
	...
	var ext = imgURL.substring(imgURL.length -3, imgURL.length);
	
	if (ext && /^(jpg|png|jpeg|gif)$/.test(ext)) {
		
		$('#previewImage' + index).attr('src', imgURL);	    
	    
	    $('#images' + index).val(imgURL.replace("/upload_img/",""));
...
function InitImageUpload(index, controllerName) {
    var url = '/happyace/common/imgUpload';
    ...
    new AjaxUpload('#btnImageUpload' + index, {
        ...
        onComplete: function (file, responseText) {
            ...
            previewImageBind(index, responseText.filePath);
        }
    });
}
...
function previewImageBind(index, imgURL) {
	...
	if (ext && /^(jpg|png|jpeg|gif)$/.test(ext)) {
		...
		$('#previewImage' + index).attr('src', "http://127.0.0.1/" + imgURL);	    
